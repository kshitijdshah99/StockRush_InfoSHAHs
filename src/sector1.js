...
const SectorPage = () => {
  ...
  const token =
    "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJfaWQiOiI2NWMyN2QxMDEyNzdiNDA1ODZjMzJkMjYiLCJpYXQiOjE3MDcyNDQ4MTYsImV4cCI6MTcwNzUwNDAxNn0.6JQqJyrFrvAEdrTT-nRIcvZ2fUB9v0rAK-FN3WugRZE";
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          "http://localhost:4000/api/ser/get/Banking",
          {
            headers: {
  ...
  const stockList = [
    { name: "State Bank of India", id: "65b109261c94e053c42886b2" },
    { name: "ICICI Bank", id: "65b109521c94e053c42886b4" },
    { name: "Kotak Mahindra Bank", id: "65b109661c94e053c42886b6" },
    { name: "Union Bank of India", id: "65b1097a1c94e053c42886b8" },
    { name: "HDFC Bank", id: "65b109861c94e053c42886ba" },
  ];
  ...
  const handleBuy = async (companyId, index) => {
    try {
      ...
      setCompanies(response.data.stock_data);
    } catch (error) {
          ...
          <tbody>
            {stockList.map((stock, index) => (
              <tr key={index}>
                <td>{stock.name}</td>
                <td>
                  <button onClick={() => handleBuy(stock.id, index)}>
                    Buy
                  </button>
                </td>
                <td>
                  <input
                    ...
                    min="0"
                    onChange={(event) => handleBuyQuantityChange(index, event)}
                  />
                </td>
                <td>{companies[index].current_price * 1.05}</td>
                <td>
                  {buyQuantities[index] * companies[index].current_price * 1.05}
                </td>
                <td>
                  <button onClick={() => handleSell(stock.id, index)}>
                    Sell
                  </button>
                </td>
                <td>
                  <input
                    type="number"
                    value={sellQuantities[index]}
                    min="0"
                    onChange={(event) => handleSellQuantityChange(index, event)}
                  />
                </td>
                <td>{companies[index].current_price}</td>
                <td>
                  {sellQuantities[index] * companies[index].current_price}
                </td>
              </tr>
            ))}
          </tbody>
